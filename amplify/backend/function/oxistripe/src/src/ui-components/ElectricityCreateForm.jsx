...
export default function ElectricityCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    ...
    setCO2perkWh(initialValues.CO2perkWh);
    setErrors({});
  };
  const validations = {
    Source: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(new Electricity(modelFields));
          if (onSuccess) {
      ...
      {...getOverrideProps(overrides, "ElectricityCreateForm")}
      {...rest}
    >
      <TextField
        label="Source"
