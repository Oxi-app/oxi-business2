...
export default function BasketUpdateForm(props) {
  const {
    id: idProp,
    basket: basketModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = basketRecord
      ? { ...initialValues, ...basketRecord }
      : initialValues;
  ...
  const [basketRecord, setBasketRecord] = React.useState(basketModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Basket, idProp)
        : basketModelProp;
      setBasketRecord(record);
    };
    queryData();
  }, [idProp, basketModelProp]);
  React.useEffect(resetStateValues, [basketRecord]);
  const validations = {
    Barcode: [],
    Merchant: [],
    Name: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Basket.copyOf(basketRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "BasketUpdateForm")}
      {...rest}
    >
      <TextField
        label="Barcode"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || basketModelProp)}
