...
export default function MaterialsUpdateForm(props) {
  ...
  const resetStateValues = () => {
    const cleanValues = materialsRecord
      ? { ...initialValues, ...materialsRecord }
      : initialValues;
    setMaterial(cleanValues.Material);
    setCO2perkg(cleanValues.CO2perkg);
    setErrors({});
  };
  const [materialsRecord, setMaterialsRecord] =
    React.useState(materialsModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Materials, idProp)
        : materialsModelProp;
      setMaterialsRecord(record);
    };
    queryData();
  }, [idProp, materialsModelProp]);
    ...
    >
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || materialsModelProp)}
