...
export default function FuelUpdateForm(props) {
  const {
    id: idProp,
    fuel: fuelModelProp,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = fuelRecord
      ? { ...initialValues, ...fuelRecord }
      : initialValues;
    setType(cleanValues.Type);
    setCO2perkg(cleanValues.CO2perkg);
    setErrors({});
  };
  const [fuelRecord, setFuelRecord] = React.useState(fuelModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Fuel, idProp)
        : fuelModelProp;
      setFuelRecord(record);
    };
    queryData();
  }, [idProp, fuelModelProp]);
  React.useEffect(resetStateValues, [fuelRecord]);
  const validations = {
    Type: [],
    CO2perkg: [],
  };
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Fuel.copyOf(fuelRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "FuelUpdateForm")}
      {...rest}
    >
      <TextField
        label="Type"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || fuelModelProp)}
